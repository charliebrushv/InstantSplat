...
from instant_splat.scene.colmap_loader import (
...
from instant_splat.utils.graphics_utils import getWorld2View2, focal2fov, fov2focal
import numpy as np
import json
import copy
import cv2
...
def loadCameras(poses, viewpoint_stack):
    """Load optimized or interpolated camera poses into viewpoint_stack"""
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, eval=False):
    cam_infos = []
    poses = []
...
<<<<<<< HEAD:instant_splat/scene/dataset_readers.py
def readColmapSceneInfo(path, images, eval, args, opt, llffhold=2):
    ...
    if eval and opt.get_video == False:
        ...
        cameras_extrinsic_file = os.path.join(
            path, "init_test_pose/sparse/0", "images.txt"
        )
=======
...
>>>>>>> upstream/main:scene/dataset_readers.py
    else:
